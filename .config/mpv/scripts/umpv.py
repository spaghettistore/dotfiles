...
def send_files_to_mpv(conn: socket.socket | BinaryIO, files: Iterable[str]) -> None:
    try:
        ...
        for f in files:
            ...
            # 'loadfile <url> insert-next' will insert the file into the
            # playlist, directly after the current entry.
            # Look at 'man mpv' and search for 'loadfile <url>' for more
            # options.
            # 'replace' is the default, 'append-play' adds to end of playlist
            send(f'raw loadfile "{f}" insert-next\n'.encode())
    except Exception:
